...
为避免只停留在静态 HTML，本次交付采用“前后端分离”产品形态：

1. **Backend API（FastAPI）**：统一承载 `build/model/diff/ci` 能力。
2. **Frontend Studio（React）**：通过 API 交互触发分析与展示结果。
3. **Engine（tools/archsync）**：继续作为核心分析内核与 CLI。

这样既保留 CLI/CI 能力，又能给团队提供真正可用的可视化审查工作台。
